  ...
  return (
    <div style={{ maxWidth: 880, margin: "0 auto", padding: "26px 28px 130px" }}>
      ...
      {logs.length === 0 ? (
        ...
        logs.map((e, i) => <Row key={e.id} e={e} no={String(i + 1).padStart(2, "0")} also={relationNote(e, c.id)} onEdit={onEdit} onDelete={onDelete} />)
      )}
      ...
      {wishes.length > 0 && (
        <>
          ...
          {wishes.map((e) => <Row key={e.id} e={e} no="☆" also={relationNote(e, c.id)} onEdit={onEdit} onDelete={onDelete} />)}
        </>
      )}
